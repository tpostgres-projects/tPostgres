 ...
 * plugin_profiler.c
 *
 *	  Profiling plugin for PL/TSQL instrumentation
 *
 *		This plugin provides a profiler for PL/TSQL functions.  For every
 *		line of PL/TSQL code, we aggregate the following performance
 *		counters:
 ...
 *  invocation of a PL/TSQL function).
 *
...
void load_plugin( PLTSQL_plugin * hooks );

/**********************************************************************
 * Helper function prototypes
 **********************************************************************/
static void 		  profiler_init( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		  profiler_func_beg( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		  profiler_func_end( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		  profiler_stmt_beg( PLTSQL_execstate * estate, PLTSQL_stmt * stmt );
static void 		  profiler_stmt_end( PLTSQL_execstate * estate, PLTSQL_stmt * stmt );

static perStmtStats *getStatsForStmt( PLTSQL_execstate * estate, PLTSQL_stmt * stmt );
static void          dumpStats( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		 dumpStatsTable( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		 dumpStatsXML( PLTSQL_execstate * estate, PLTSQL_function * func );
static bool 		 statsAlreadyExist( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		 updateStats( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		 insertStats( PLTSQL_execstate * estate, PLTSQL_function * func );
 ...
 *  fills in the function pointers in the given PLTSQL_plugin struct
 * -------------------------------------------------------------------
 */

static PLTSQL_plugin plugin_funcs = { profiler_init, profiler_func_beg, profiler_func_end, profiler_stmt_beg, profiler_stmt_end };

void _PG_init( void )
{
	PLTSQL_plugin ** var_ptr = (PLTSQL_plugin **) find_rendezvous_variable( "PLTSQL_plugin" );
	...
	*var_ptr = &plugin_funcs;
	 ...
	 *
	 *		 We can enounter an existing variable if the user calls a PL/TSQL function (thus loading this plugin),
	 *		 and then LOAD's the plugin manually (throwing out the old incarnation of the plugin and loading a new
	 *		 instance), and finally invokes a PL/TSQL function again. 
	 */
	...
	NOTE: our XML support is pretty flawed at the moment so we will just disable
		...
		DefineCustomStringVariable( "pltsql.profiler_filename",
									"Pathname of PL/TSQL profile file",
									NULL,
									&xmlFileName,
									PGC_USERSET,
									NULL, 
		...
		DefineCustomStringVariable( "pltsql.profiler_tablename",
									"Name of PL/TSQL profile table",
									NULL,
									&statsTableName,
                                                                        NULL,
									PGC_USERSET,
 ...
 * profiler_init()
 *
 *	This hook function is called by the PL/TSQL interpreter when a 
...
static void profiler_init( PLTSQL_execstate * estate, PLTSQL_function * func )
{
	...
	/* If we don't have a value for pltsql.profiler_filename, just go home */
	...
	/*
	 * The PL/TSQL interpreter provides a void pointer (in each stack frame) that's reserved
 ...
 * profiler_stmt_beg()
 *
 *	This hook function is called by the PL/TSQL interpreter just before
 ...
 * profiler_stmt_end()
 *
 *	This hook function is called by the PL/TSQL interpreter just after
 ...
 * dumpStats()
 *
 *	dumpStats() is called at the end of each PL/TSQL function - it 
	...
	if( SPI_connect() < 0 )
	{
		ereport(ERROR,
				(errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
				 errmsg("plugin_tsql_profiler: could not connect to SPI")));
	}
...
static void updateStats( PLTSQL_execstate * estate, PLTSQL_function * func )
{
	...
	for( lineNo = 0; lineNo < profilerInfo->lineCount; ++lineNo )
	{
		...
		if( SPI_execp( updatePlan, values, NULL, 1 ) != SPI_OK_UPDATE )
		{
			ereport(ERROR,
					(errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
					 errmsg("plugin_tsql_profiler: error updating profiler data")));
		}
	}
...
static void insertStats( PLTSQL_execstate * estate, PLTSQL_function * func )
{
	...
	for( lineNo = 0; lineNo < profilerInfo->lineCount; ++lineNo )
	{
		...
		if( SPI_execp( insertPlan, values, NULL, 1 ) != SPI_OK_INSERT )
		{
			ereport(ERROR,
					(errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
					 errmsg("plugin_tsql_profiler: error inserting profiler data")));
		}
...
static bool statsAlreadyExist( PLTSQL_execstate * estate, PLTSQL_function * func )
{
	...
	if( SPI_execp( selectPlan, values, NULL, 1 ) != SPI_OK_SELECT )
	{
			ereport(ERROR,
					(errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
					 errmsg("plugin_tsql_profiler: error querying profiler data")));
	}
...
static bool tableExists( const char * qualifiedName )
{
	...
	if( OidIsValid( RangeVarGetRelid( relVar, NoLock, true )))
		return true;
	else
		return false;
}
