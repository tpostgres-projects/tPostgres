/**********************************************************************
 * plugin_debugger.c	- Debugger for the PL/TSQL procedural language
 *
...
typedef struct
{
    PLTSQL_function *	func;				/* Function definition										*/
	...
	void 			 (* assign_expr)( PLTSQL_execstate *estate, PLTSQL_datum *target, PLTSQL_expr *expr );
#if INCLUDE_PACKAGE_SUPPORT
 ...
 **********************************************************************/
static void 		 dbg_startup( PLTSQL_execstate * estate, PLTSQL_function * func );
static void 		 dbg_newstmt( PLTSQL_execstate * estate, PLTSQL_stmt * stmt );
static void 		 initialize_plugin_info( PLTSQL_execstate * estate, PLTSQL_function * func );

static char       ** fetchArgNames( PLTSQL_function * func, int * nameCount );
...
static PLTSQL_var * find_var_by_name( const PLTSQL_execstate * estate, const char * var_name, int lineno, int * index );
static void 	     dbg_printvar( PLTSQL_execstate * estate, const char * var_name, int lineno );
...
static bool 		 is_datum_visible( PLTSQL_datum * datum );
static bool			 is_var_visible( PLTSQL_execstate * frame, int var_no );
static Oid			 funcGetOid(  PLTSQL_function * func );
static bool			 datumIsNull(PLTSQL_datum *datum);
...
static bool          varIsArgument( const PLTSQL_execstate * frame, int varNo );
 ...
 *	loaded.  It creates a rendezvous variable (PLTSQL_plugin) that 
 *	the PL/TSQL interpreter can use to find this plugin. By mutual
 *	agreement, the PL/TSQL interpreter expects to find a PLTSQL_plugin
 *	structure in that rendezvous variable.
 */

static PLTSQL_plugin plugin_funcs = { dbg_startup, NULL, NULL, dbg_newstmt, NULL };
...
static const char * plugin_name  = "PLTSQL_plugin";
#define OID_DEBUG_FUNCTION	pltsql_oid_debug
...
void _PG_init( void )
{
	PLTSQL_plugin ** var_ptr = (PLTSQL_plugin **) find_rendezvous_variable( plugin_name );
 ...
 *
 *	This function is invoked by the server when the PL debugger is
 *	unloaded.  It clears out the PLTSQL_plugin rendezvous variable.
 */
...
/*
 * CREATE OR REPLACE FUNCTION xxx_oid_debug( functionOID OID ) RETURNS INTEGER AS 'pltsql' LANGUAGE C;
 */
...
static void dbg_send_src( PLTSQL_execstate * frame, char * command  )
{
    HeapTuple			tup;
    char				*procSrc;
	char				*funcName;
 ...
 * find_var_by_name()
 *
 *	This function returns the PLTSQL_var pointer that corresponds to 
...
static PLTSQL_var * find_var_by_name( const PLTSQL_execstate * estate, const char * var_name, int lineno, int * index )
{
    ...
    PLTSQL_function * func     = dbg_info->func;
    int                i;
...
static PLTSQL_datum * find_datum_by_name( const PLTSQL_execstate * frame, const char * var_name, int lineNo, int * index )
{
	...
	if( var_name[0] == '@' )
	{
		...
		{
			PLTSQL_datum * datum = dbg_info->package->datums[varIndex];
	...
	for( i = 0; i < frame->ndatums; ++i )
	{
		...
		switch( frame->datums[i]->dtype )
		{
			...
			{
				PLTSQL_variable * var = (PLTSQL_variable *)frame->datums[i];
			...
			case PLTSQL_DTYPE_EXPR:
#if (PG_VERSION_NUM <= 80400)
			case PLTSQL_DTYPE_TRIGARG:
#endif
			{
				break;
			}
...
static void print_var( const PLTSQL_execstate * frame, const char * var_name, int lineno, const PLTSQL_var * tgt )
{
    char	     	 * extval;
    HeapTuple	       typeTup;
...
static void print_row( const PLTSQL_execstate * frame, const char * var_name, int lineno, const PLTSQL_row * tgt )
{


}

static void print_rec( const PLTSQL_execstate * frame, const char * var_name, int lineno, const PLTSQL_rec * tgt )
{
	int		attNo;
...
static void print_recfield( const PLTSQL_execstate * frame, const char * var_name, int lineno, const PLTSQL_recfield * tgt )
{
...
static void dbg_printvar( PLTSQL_execstate * estate, const char * var_name, int lineno )
{
	...
	PLTSQL_variable * generic = NULL;

	/* Try to find the given variable */

    if(( generic = (PLTSQL_variable*) find_var_by_name( estate, var_name, lineno, NULL )) == NULL )
    {
	...
	{
		case PLTSQL_DTYPE_VAR:
			print_var( estate, var_name, lineno, (PLTSQL_var *) generic );
			break;

		case PLTSQL_DTYPE_ROW:
			print_row( estate, var_name, lineno, (PLTSQL_row *) generic );
			break;

		case PLTSQL_DTYPE_REC:
			print_rec( estate, var_name, lineno, (PLTSQL_rec *) generic );
			break;

		case PLTSQL_DTYPE_RECFIELD:
			print_recfield( estate, var_name, lineno, (PLTSQL_recfield *) generic );
			break;
 ...
 *
 *	In a PL/TSQL function/procedure you can declare many variables with
...
static void mark_duplicate_names( const PLTSQL_execstate * estate, int var_no )
{
    dbg_ctx * dbg_info = (dbg_ctx *)estate->plugin_info;
	...
	if( estate->datums[var_no]->dtype != PLTSQL_DTYPE_VAR )
	{
	...
	{
		PLTSQL_var * var	   = (PLTSQL_var *)estate->datums[var_no];
		...
		for( i = 0; i < estate->ndatums; ++i )
		{
			if( i != var_no )
			{
				...
				var = (PLTSQL_var *)estate->datums[i];
...
static void completeFrame( PLTSQL_execstate * frame )
{
    dbg_ctx 		 * dbg_info = (dbg_ctx *)frame->plugin_info;
    PLTSQL_function * func     = dbg_info->func;
    int		           i;
...
static char * get_text_val( PLTSQL_var * var, char ** name, char ** type )
{
	HeapTuple	       typeTup;
    Form_pg_type       typeStruct;
    FmgrInfo	       finfo_output;
 ...
 *	 whenever send_stack() finds a PL/TSQL call in the stack (remember,
 *	 the call stack may contain stack frames for functions written in 
 *	 other languages like PL/Tcl).
 */

static void send_pltsql_frame( PLTSQL_execstate * estate )
{

#if (PG_VERSION_NUM >= 80500)
	PLTSQL_function  * func     = estate->func;
#else
	PLTSQL_function  * func     = estate->err_func;
#endif
	PLTSQL_stmt	  * stmt 	 = estate->err_stmt;
	int					argNameCount;
	...
	appendStringInfo( result, "%s:%d:%d:", func->fn_signature, funcGetOid( func ), stmt->lineno );
	...
	for( arg = 0; arg < func->fn_nargs; ++arg )
	{
		...
		PLTSQL_datum		*argDatum = (PLTSQL_datum *)estate->datums[index];
		char 				*value;
		...
		if( datumIsNull( argDatum ))
			...
			value = get_text_val((PLTSQL_var*)argDatum, NULL, NULL );
 ...
 *	 each PL/TSQL stack frame that we find, we send the function name,
...
static void send_stack( dbg_ctx * dbg_info )
{
	...
	for( entry = error_context_stack; entry; entry = entry->previous )
	{
		...
		{
			send_pltsql_frame((PLTSQL_execstate *)( entry->arg ));
		}
	}
...
static void setBreakpoint( const PLTSQL_execstate * frame, char * command )
{
	/* 
...
static void clearBreakpoint( PLTSQL_execstate * frame, char * command )
{
	/* 
...
static void send_breakpoints( PLTSQL_execstate * frame )
{
	dbg_ctx 		* dbg_info = (dbg_ctx *)frame->plugin_info;
	Breakpoint      * breakpoint;
...
static void send_vars( PLTSQL_execstate * frame )
{
	int       i;
	dbg_ctx * dbg_info = (dbg_ctx *)frame->plugin_info;
	...
	for( i = 0; i < frame->ndatums; i++ )
	{
		if( is_var_visible( frame, i ))
		{
			switch( frame->datums[i]->dtype )
			{
				case PLTSQL_DTYPE_VAR:
				{
					PLTSQL_var * var = (PLTSQL_var *) frame->datums[i];
					char        * val;
					char		* name = var->refname;
					bool		  isArg = varIsArgument( frame, i );

					if( datumIsNull((PLTSQL_datum *)var ))
						val = "NULL";
					else
			...
			FIXME: implement other types
				...
				case PLTSQL_DTYPE_REC:
				{
					PLTSQL_rec * rec = (PLTSQL_rec *) frame->datums[i];
					int		      att;
					char        * typeName;
	...
	if( dbg_info->package != NULL )
	{
		...
		{
			PLTSQL_datum * datum = package->datums[varIndex];
...
static PLTSQL_execstate * select_frame( dbg_ctx * dbg_info, PLTSQL_execstate * frameZero, int frameNo )
{
	ErrorContextCallback * entry;
	...
	for( entry = error_context_stack; entry; entry = entry->previous )
	{
		if( entry->callback == dbg_info->error_callback )
		{
			if( frameNo-- == 0 )
			{
				PLTSQL_execstate *estate = entry->arg;
 ...
 * dbg_startup()
 *
 *	This function is called the PL/TSQL executor each time a function is
 *	invoked.  pltsql_dbg_startup() returns quickly if user has defined no break-
 *  points.  
 *
...
static void initialize_plugin_info( PLTSQL_execstate * estate,
									PLTSQL_function * func )
{
	 ...
	 * PL/TSQL (or SPL) stack frames (hokey, but it works).  We use
...
static void do_deposit( PLTSQL_execstate * frame, const char * command )
{
	dbg_ctx       *dbg_info   = frame->plugin_info;
	PLTSQL_datum *target     = NULL;
	...
	*lineno = '\0';
    ...
    /*
	 * Note: we must create a dynamically allocated PLTSQL_expr here - we 
	 ...
	 *	     transaction so we don't free it out from under spl_pltsql_xact_cb()
	 */

	expr = (PLTSQL_expr *) palloc( sizeof( *expr ));
...
static void send_cur_line( PLTSQL_execstate * estate, PLTSQL_stmt * stmt )
{
    dbg_ctx 		 * dbg_info = (dbg_ctx *)estate->plugin_info;
	PLTSQL_function * func     = dbg_info->func;

	dbg_send( per_session_ctx.client_w, "%d:%d:%s", funcGetOid( func ), stmt->lineno+1, func->fn_signature );
}
...
static bool isFirstStmt( PLTSQL_stmt * stmt, PLTSQL_function * func )
{
 ...
 * dbg_newstmt()
 *
 *	The PL/TSQL executor calls pltsql_dbg_newstmt() just before executing each
 *	statement.  
 *
 *	This function is the heart of the debugger.  If you're single-stepping,
 *	or you hit a breakpoint, pltsql_dbg_newstmt() sends a message to the debugger
 ...
 *		  if you're running PL/TSQL code without a debugger, you notice no
 *		  performance penalty.
 */

static void dbg_newstmt( PLTSQL_execstate * estate, PLTSQL_stmt * stmt )
{
	PLTSQL_execstate * frame = estate;
	...
	else
	{
		...
		/*
		 * The PL/TSQL compiler inserts an automatic RETURN stat1ement at the
		...
		if( dbg_info->stepping )
		{
			 ...
			 * from the user that would let us execute this PL/TSQL statement.
			 */
			while( need_more )
			{
				//Fog Bug: 2079 
 ...
 *	TODO: consider datatypes other than PLTSQL_DTYPE_VAR as well
 */
static bool datumIsNull(PLTSQL_datum *datum)
{
	switch (datum->dtype)
 ...
 * funcGetOid()
 *
 *	Returns the OID of a function/procedure given a PLTSQL_function 
 *  pointer.
 */
